...
export const createAppointment = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    // Auto-assign default doctor room if no explicit roomId supplied
    ...
    if (!resolvedRoomId) {
      ...
      if (!docRoom) {
        res.status(400).json({ error: 'Médico selecionado não possui salas vinculadas. Especifique uma sala.' });
        return;
      }
      resolvedRoomId = docRoom.roomId;
    }
...
export const getAvailableSlots = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    if (!doctorId || !date) {
      res.status(400).json({ error: 'Médico e data são obrigatórios.' });
      return;
    }
    ...
    const schedule = JSON.parse(doctor.schedule_config);
    ...
    const busySlots = appointments.map(a => a.appointment_datetime.toISOString().substr(11, 5));
