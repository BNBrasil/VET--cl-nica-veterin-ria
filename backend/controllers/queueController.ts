...
const ticketSchema = z.object({
  ...
  targetType: z.enum(['ESPECIALIDADE', 'SALA', 'EXAME', 'TRIAGEM', 'MEDICO']).optional().nullable(),
  targetName: z.string().optional().nullable(),
  roomId: z.string().uuid().optional().nullable(),
  examTypeId: z.string().uuid().optional().nullable(),
});

export const generateTicket = async (req: Request, res: Response): Promise<void> => {
  try {
    const { classification, animalId, animalName, targetType, targetName, roomId, examTypeId } = ticketSchema.parse(req.body);
    ...
    const ticketRaw = await prisma.queueNumber.create({
      data: {
        code: generatedCode,
        classification,
        status: 'AGUARDANDO',
    ...
    // Re-fetch com include para retornar dados completos ao frontend
    const ticket = await prisma.queueNumber.findUnique({
      where: { id: ticketRaw.id },
      include: {
        animal: {
          select: {
            name: true,
    ...
    await logAction(actor, 'GERAR_SENHA', 'queue_numbers', ticketRaw.id);
...
export const callNextTicket = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    // Check if the caller has a room assigned via doctor profile
    ...
    if (doctor && doctor.doctor_rooms.length > 0) {
      roomId = doctor.doctor_rooms[0].roomId;
    }
    ...
    const updated = await prisma.queueNumber.update({
      ...
      data: { 
        ...
        targetType: 'MEDICO',
        targetName: doctor?.user?.name || undefined
      },
      include: {
        animal: {
          select: { 
...
export const updateTicketStatus = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    // If status is being changed to CHAMADA, try to assign room
    ...
    if (status === 'CHAMADA') {
      ...
      if (doctor) {
        ...
        if (doctor.doctor_rooms.length > 0) {
          roomId = doctor.doctor_rooms[0].roomId;
        }
      }
    }
    ...
    const updated = await prisma.queueNumber.update({
      ...
      data: { 
        status,
        roomId: roomId || undefined,
        targetType: doctorName ? 'MEDICO' : undefined,
        targetName: doctorName || undefined
      },
      include: {
        animal: {
          select: { 
